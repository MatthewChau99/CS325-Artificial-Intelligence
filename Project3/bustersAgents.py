...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState):
        ...
        # the most likely position the ghosts would move to
        mostLikelyPos = [belief.argMax() for belief in livingGhostPositionDistributions]

        # the closest to the current pacman position
        closestPos = min(mostLikelyPos, key=lambda pos: self.distancer.getDistance(pacmanPosition, pos))

        # the action to the closest position
        return min(legal, key=lambda action: self.distancer.getDistance(Actions.getSuccessor(pacmanPosition, action), closestPos))